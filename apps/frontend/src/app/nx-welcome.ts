...
import { getHello } from './clients/hello.client';
...
export class NxWelcome {
  ...
  readonly helloMessage = signal<string | null>(null);

  constructor() {
    this.loadHelloMessage();
  }

  private loadHelloMessage() {
    getHello(this.http).subscribe({
      next: (response) => this.helloMessage.set(response.message),
      error: () => this.helloMessage.set('Error calling API'),
    });
  }
}
